...
public class DBManager {
	...
	public Map<String, List<Object>> addSessions(org.web.beans.SessionInfo sessionInfo) {
		...
		List<org.web.beans.Session> existingSessions = getAllScheduledSessions(sessionInfo.getNetID(),
				sessionInfo.getStartDate(), sessionInfo.getEndDate());
	...
	public List<Object> getScheduledSessionsUpto(String netID, String date) {
		...
		SQLQuery q = (SQLQuery) session.getNamedQuery("getSCHDSessionsUpto").setString("netID", netID).setString("date",
				date);
	...
	public List<User> startSession(Integer sessionID) {
		...
		Query usersQ = session.createQuery(
				"from User where netID in (select netID from Appointment where sessionID=:sessionID order by slotNo asc)");
	...
	public User getUserForAppointment(Integer appointmentID) {
		...
		Query q = session.createQuery(
				"from User where netID in (select netID from Appointment where appointmentID=:appointmentID)");
	...
	public Response createAppointment(int sessID, String netID, Date date, String appointmentReason) {
		...
		SQLQuery q = (SQLQuery) hSession.getNamedQuery("getNoShowSessions").setString("netID", netID);
		q.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
		res = q.list();

		if (res != null && res.size() > 0) {
			hSession.close();
			r.setMessage("You have a No Show policy enforced");
			return r;
		}
		...
		Criteria criteria = hSession.createCriteria(org.web.beans.Session.class);
		criteria.add(Restrictions.eq("sessionID", sessID));
		sessionData = criteria.list();

		List<org.web.beans.Appointment> Appointments = null;
		Criteria criteriaApp = hSession.createCriteria(org.web.beans.Appointment.class);
		criteriaApp.add(Restrictions.eq("netID", netID));
		criteriaApp.add(Restrictions.eq("status", "SCHEDULED"));
		...
		if ((flag == 1) && (sessIds != null)) {
			...
			Criteria criteriaConf = hSession.createCriteria(org.web.beans.Session.class);
			...
			if (sess3 != null) {
				...
				if (sess3.getSlotCounter() < sess3.getNoOfSlots()) {
					...
					hSession.createQuery(hqlUpdate).setParameter("counter", counter)
							.setParameter("sessionid", sess3.getSessionID()).executeUpdate();

					txn.commit();
					hSession.close();

					allAppointments = getScheduledApps(netID);
	...
	public List<Object> cancelAppointments(String netID, Integer[] appointmentIDs) {
		Session session = factory.openSession();
		...
		try {
			...
			Query q = session.createQuery(
					"update Appointment set status = :stat where netID = :netID and sessionID in (:appointmentIDs)");
		...
		try {
			...
			Query q1 = nsession
					.createQuery("update Session set slotCounter = slotCounter-1 where sessionID in (:appointmentIDs)");
	...
	public List<Object> getSessionAppointments(Integer sessionID) {
		...
		SQLQuery q = (SQLQuery) session.getNamedQuery("getAllSessionAppointments").setInteger("sessionID", sessionID);
		q.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
	...
	public List<User> cancelSession(Integer sessionID, String cancelReason) {
		...
		try {
			tx = session.beginTransaction();
			Query cancelApptsQ = session
					.createQuery("update Appointment set status = 'CANCELLED' where sessionID=:sessionID");
		...
		Query usersQ = session
				.createQuery("from User where netID in (select netID from Appointment where sessionID=:sessionID)");
		List<User> users = usersQ.setParameter("sessionID", sessionID).list();
	...
	public List<Object> getAllAnnouncements(String startDate, String endDate, String branch, String netID) {
		...
		SQLQuery q = (SQLQuery) session.createSQLQuery(sql).setString("startDate", startDate).setString("endDate",
				endDate);
		q.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
